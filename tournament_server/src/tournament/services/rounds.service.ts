...
export class RoundsService {
  ...
  async create(dto: CreateRoundDto) {
    ...
    if (!match) {
      throw Error(
        `Match with id ${dto.matchId} not found. Insert round failed`,
      );
    }
    ...
    if (!song) {
      throw new NotFoundException(
        `Song with id ${dto.songId} not found. Insert round failed`,
      );
    }

    newRound.match = match;
    newRound.song = song;
    newRound.disabledPlayerIds = dto.disabledPlayerIds ?? [];
  ...
  async update(id: number, dto: UpdateRoundDto) {
    ...
    if (!round) {
      throw new NotFoundException(
        `Round with id ${id} not found. Update round failed`,
      );
    }
    ...
    if (dto.matchId) {
      ...
      if (!match) {
        throw new NotFoundException(
          `Match with id ${dto.matchId} not found. Update round failed`,
        );
      }
      dto.match = match;
      delete dto.matchId;
    ...
    if (dto.songId) {
      ...
      if (!song) {
        throw new NotFoundException(
          `Song with id ${dto.songId} not found. Update round failed`,
        );
      }
      dto.song = song;
      delete dto.songId;
