import { createUser, getRegistrationPrefill, RegistrationPrefill } from '../services/user.api';
...
export default function RegisterCompontent() {
  ...
  const [lookupAlert, setLookupAlert] = useState<{ type: 'success' | 'warning'; message: string } | null>(null);
  ...
  const validateAccountFields = (): boolean => {
    ...
    if (!vname) {
      setErrMsg('Username must be 3-23 characters and use letters, numbers, "-" or "_".');
      return false;
    }
    if (!vemail) {
  ...
  const handleLookup = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const message = errorMessage(error, 'Unable to validate preregistration gamer tag.');
    ...
    <>
      {success ? (
        ...
        <section className="w-full flex justify-center px-4">
          <p ref={errRef} className={errMsg ? 'errmsg' : 'offscreen'} aria-live="assertive" tabIndex={-1}>
            {errMsg}
          </p>
          ...
          >
            <div className="flex items-center justify-center gap-3 text-xs uppercase tracking-wider">
              <span className={step === 1 ? 'font-bold text-white' : 'text-white/60'}>1. Account</span>
              <span className="text-white/40">/</span>
              <span className={step === 2 ? 'font-bold text-white' : 'text-white/60'}>2. Confirm</span>
            </div>
            ...
            {step === 2 && lookupAlert && (
              ...
              >
                {lookupAlert.type === 'success' && registrationPrefill && (
                  <>
                    <div className="text-base font-semibold leading-tight">Preregistration found</div>
                    <div className="mt-1 text-white/90">{lookupAlert.message}</div>
                    <div className="mt-2 text-white/90">Gamer tag: {registrationPrefill.gamerTag ?? 'Unknown'}</div>
                    <div className="text-white/90">Ticket code: {registrationPrefill.ticketCode ?? 'Unknown'}</div>
                    <div className="text-white/90">Attending as: {registrationPrefill.attendingAs ?? 'Unknown'}</div>
                  </>
                )}
                {lookupAlert.type !== 'success' && <div>{lookupAlert.message}</div>}
              </div>
            )}
            ...
            {step === 1 && (
              <>
                ...
                <div>
                  ...
                  />
                  <p id="matchnote" className={!validMatch && matchPwd ? 'instructions' : 'offscreen'}>
              ...
              <>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    {divisionLoading && <div className="text-sm text-gray-500">Loading divisions...</div>}
            ...
            <div className="flex gap-2">
              {step === 1 ? (
                <button
                  type="submit"
                  disabled={!validName || !validEmail || !validPwd || !validMatch || prefillLoading}
                ...
                <>
                  ...
                  <button type="submit" className="bg-lighter text-white p-2 rounded-lg w-2/3 mt-2">
                    Submit account
                  </button>
                </>
              )}
